...
func TestWSMonitor_GetCurrentKlines_StaleDataDetection(t *testing.T) {
	...
	// TDD EXPECTATION: Should detect that ReceivedAt is 6 hours ago and automatically refresh from API
	// Our implementation auto-refreshes instead of returning error (better UX)
	if err != nil {
		t.Fatalf("Should auto-refresh stale data, got error: %v", err)
	}

	if klines == nil || len(klines) == 0 {
		t.Fatal("Should return fresh klines from API after detecting stale data")
	}

	// Verify data was refreshed (should have more than 2 klines from API)
	if len(klines) < 10 {
		t.Errorf("Expected at least 10 klines from API refresh, got %d", len(klines))
	}

	t.Logf("✅ Test PASSED: Stale data correctly detected and auto-refreshed from API (got %d klines)", len(klines))
}
...
func TestWSMonitor_GetCurrentKlines_BoundaryCase(t *testing.T) {
	...
	// Should auto-refresh data older than 15 minutes
	if err != nil {
		t.Fatalf("Should auto-refresh data older than 15 minutes, got error: %v", err)
	}

	if klines == nil || len(klines) == 0 {
		t.Fatal("Should return fresh klines from API")
	}

	t.Logf("✅ Test PASSED: Data older than 15min correctly auto-refreshed (got %d klines)", len(klines))
}
